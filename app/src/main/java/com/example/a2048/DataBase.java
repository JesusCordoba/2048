...
public class DataBase extends SQLiteOpenHelper {
    ...
    public ArrayList<Score> getScores() {
        ...
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery("select * from " + DATABASE_TABLE + " GROUP BY score ORDER BY score DESC", null);
    ...
    public int getBestScore(String usuario) {
        int best_score = 0;
        SQLiteDatabase db = this.getWritableDatabase();
        //"select * from " + DATABASE_TABLE + "WHERE player LIKE "+ usuario +" GROUP BY score ORDER BY score DESC LIMIT 1"
        Cursor cursor = db.rawQuery("select * from " + DATABASE_TABLE + " where player LIKE '"+ usuario +"' GROUP BY score ORDER BY score DESC LIMIT 1", null);
        if (cursor.getCount() != 0) {
            if (cursor.moveToFirst()) {
                do {
